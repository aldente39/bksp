...
zspmat *zread_matrix_market(char *filename){
    ...
    row = (int *) malloc(nz * sizeof(int));
    col = (int *) malloc(nz * sizeof(int));
    val = (double _Complex *) malloc(nz * sizeof(double _Complex));
...
int zcoo2csr(zspmat *mat) {
    MKL_INT *n, *ja, *ia, *nnz, *row_ind, *col_ind;
    double _Complex *Acsr, *Acoo;
    //MKL_INT job[] = {1,0,0,0,*mat->nnz,0};
    n = mat->row_size;
    ja = (int *)calloc((*mat->nnz), sizeof(int));
    ia = (int *)calloc((*n + 1), sizeof(int));
    Acsr = (double _Complex *)malloc((*mat->nnz) * sizeof(double _Complex));
    Acoo = mat->value;
    nnz = mat->nnz;
    row_ind = mat->row;
    col_ind = mat->col;
